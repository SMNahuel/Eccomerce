...
const randomNumber = (min, max) =>
  (Math.random() * (max - min) + (min + 0.5)) | 0;
const randomMessage = (val) =>
  lorem.slice(0, (Math.random() * (val - 1) + 1.5) | 0);
...
const createImages = () => {
  categories.forEach(
    (category) =>
      (p = p.then(() => Image.create({ fileName: category + ".png" })))
  );
};

const createProfileImages = () => {
  users.forEach(
    (user, i) => (p = p.then(() => Image.create({ fileName: i + 1 + ".jpg" })))
  );
};
